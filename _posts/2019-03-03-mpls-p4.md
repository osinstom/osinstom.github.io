...
![mpls-network.png]({{site.baseurl}}/_posts/mpls-network.png)
...
The _push_mpls()_ action adds the MPLS header to a packet and sets  a value of MPLS fields. Moreover, it modifies etherType to indicate the MPLS protocol. The _pop_mpls()_ action removes the MPLS header and reverts a value of etherType to indicate the IPv4 protocol. The _swap_mpls()_ action just changes a value of the MPLS label and decrements the TTL value. 

The first table that handles incoming packets is a _fec_table_. It classifies packets (based on the destination IP address) to the MPLS class (tunnel). If classified, a packet may be encapsulated by using the _push_mpls()_ action. 
